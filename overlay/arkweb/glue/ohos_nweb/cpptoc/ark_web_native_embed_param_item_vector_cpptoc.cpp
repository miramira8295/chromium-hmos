...
ArkWebNativeEmbedParamItemVector ArkWebNativeEmbedParamItemVectorClassToStruct(
    const std::vector<std::shared_ptr<OHOS::NWeb::NWebNativeEmbedParamItem>>&
        class_value) {
  ArkWebNativeEmbedParamItemVector struct_value = {
      .size = class_value.size(), .ark_web_mem_free_func = ArkWebMemFree};
  if (struct_value.size > 0) {
    struct_value.value = (ark_web_native_embed_param_item_t**)ArkWebMemMalloc(
        sizeof(ark_web_native_embed_param_item_t*) * struct_value.size);
    ...
    for (auto it = class_value.begin(); it != class_value.end(); it++) {
      ArkWebRefPtr<ArkWebNativeEmbedParamItem> ark_web_native_embed_param_item =
          new ArkWebNativeEmbedParamItemImpl(*it);
      struct_value.value[count] = ArkWebNativeEmbedParamItemCppToC::Invert(
