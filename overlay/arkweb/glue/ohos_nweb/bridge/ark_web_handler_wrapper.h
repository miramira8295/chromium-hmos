...
class ArkWebHandlerWrapper : public OHOS::NWeb::NWebHandler {
 public:
  ...
  void KeyboardReDispatch(std::shared_ptr<OHOS::NWeb::NWebKeyEvent> event,
  ...
  void OnAdsBlocked(const std::string& url,
                    const std::vector<std::string>& adsBlocked) override;

  void OnCursorUpdate(double x, double y, double width, double height) override;

  void ReportDynamicFrameLossEvent(const std::string& sceneId,
   ...
   *
   * @param paramDataInfo The information containing the set of all changed
   * params for the native object.
   *
   */
   ...
   */
  void OnRefreshAccessedHistoryV2(const std::string& url,
                                  bool isReload,
                                  bool isMainFrame) override;
