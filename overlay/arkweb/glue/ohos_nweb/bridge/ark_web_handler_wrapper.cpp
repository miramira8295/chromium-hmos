...
void ArkWebHandlerWrapper::OnPageIcon(const void* data,
                                      size_t width,
                                      size_t height,
...
bool ArkWebHandlerWrapper::FilterScrollEvent(const float x,
                                             const float y,
...
void ArkWebHandlerWrapper::CreateOverlay(void* data,
                                         ...
                                         int point_x,
                                         int point_y) {
  ark_web_handler_->CreateOverlay(data, len, width, height, offset_x, offset_x,
                                  rect_width, rect_height, point_x, point_y);
}
...
void ArkWebHandlerWrapper::OnOverlayStateChanged(int offset_x,
                                                 ...
                                                 int rect_width,
                                                 int rect_height) {
  ark_web_handler_->OnOverlayStateChanged(offset_x, offset_y, rect_width,
                                          rect_height);
}
...
bool ArkWebHandlerWrapper::OnOverScroll(float xOffset,
                                        float yOffset,
                                        float xVelocity,
                                        float yVelocity) {
...
void ArkWebHandlerWrapper::KeyboardReDispatch(
    std::shared_ptr<OHOS::NWeb::NWebKeyEvent> event,
    bool isUsed) {
  if (CHECK_SHARED_PTR_IS_NULL(event)) {
...
void ArkWebHandlerWrapper::OnInterceptKeyboardAttach(
    const std::shared_ptr<OHOS::NWeb::NWebCustomKeyboardHandler>
        keyboardHandler,
  ...
  ark_web_handler_->OnInterceptKeyboardAttach(
      new ArkWebCustomKeyboardHandlerImpl(keyboardHandler),
      ArkWebStringMapClassToStruct(attributes), useSystemKeyboard,
      enterKeyType);
}
...
void ArkWebHandlerWrapper::OnCursorUpdate(double x,
                                          double y,
                                          double width,
                                          double height) {
...
void ArkWebHandlerWrapper::ReportDynamicFrameLossEvent(
...
void ArkWebHandlerWrapper::OnNativeEmbedVisibilityChange(
...
bool ArkWebHandlerWrapper::OnSslErrorRequestByJSV2(
  ...
  ArkWebStringVector stCertChainData =
      ArkWebStringVectorClassToStruct(certChainData);

  bool flag = false;
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    flag = ark_web_handler_->OnSslErrorRequestByJSV2(
        nullptr, static_cast<int>(error), stCertChainData);
  } else {
    flag = ark_web_handler_->OnSslErrorRequestByJSV2(
        new ArkWebJsSslErrorResultImpl(result), static_cast<int>(error),
        stCertChainData);
...
void ArkWebHandlerWrapper::OnAccessibilityEvent(int64_t accessibilityId,
                                                int32_t eventType) {
...
void ArkWebHandlerWrapper::GetVisibleRectToWeb(int& visibleX,
                                               int& visibleY,
                                               int& visibleWidth,
                                               int& visibleHeight) {
  ark_web_handler_->GetVisibleRectToWeb(visibleX, visibleY, visibleWidth,
                                        visibleHeight);
}
...
void ArkWebHandlerWrapper::OnShowAutofillPopupV2(
    ...
    const std::vector<std::string>& menu_items) {
  ArkWebStringVector stMenuItems = ArkWebStringVectorClassToStruct(menu_items);
  ark_web_handler_->OnShowAutofillPopupV2(offsetX, offsetY, height, width,
                                          stMenuItems);
...
void ArkWebHandlerWrapper::OnAccessibilityEventV2(int64_t accessibilityId,
                                                  int32_t eventType,
                                                  const std::string& argument) {
  ArkWebString stArgument = ArkWebStringClassToStruct(argument);
  ark_web_handler_->OnAccessibilityEventV2(accessibilityId, eventType,
                                           stArgument);
  ArkWebStringStructRelease(stArgument);
}

bool ArkWebHandlerWrapper::OnNestedScroll(float& x,
                                          float& y,
                                          float& xVelocity,
                                          float& yVelocity,
                                          bool& isAvailable) {
  return ark_web_handler_->OnNestedScroll(x, y, xVelocity, yVelocity,
                                          isAvailable);
}
...
void ArkWebHandlerWrapper::OnPip(int status,
                                 ...
                                 int width,
                                 int height) {
  ark_web_handler_->OnPip(status, delegate_id, child_id, frame_routing_id,
                          width, height);
}

bool ArkWebHandlerWrapper::OnBeforeUnloadByJSV2(
    const std::string& url,
    const std::string& message,
    bool isReload,
    std::shared_ptr<OHOS::NWeb::NWebJSDialogResult> result) {
  ...
  bool flag = false;
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    flag = ark_web_handler_->OnBeforeUnloadByJSV2(stUrl, stMessage, isReload,
                                                  nullptr);
  } else {
    flag = ark_web_handler_->OnBeforeUnloadByJSV2(
        stUrl, stMessage, isReload, new ArkWebJsDialogResultImpl(result));
  }
...
void ArkWebHandlerWrapper::OnNativeEmbedMouseEvent(
    std::shared_ptr<OHOS::NWeb::NWebNativeEmbedMouseEvent> mouseEvent) {
  if (CHECK_SHARED_PTR_IS_NULL(mouseEvent)) {
    ark_web_handler_->OnNativeEmbedMouseEvent(nullptr);
    return;
  }

  ark_web_handler_->OnNativeEmbedMouseEvent(
      new ArkWebNativeEmbedMouseEventImpl(mouseEvent));
}
...
bool ArkWebHandlerWrapper::OnAllSslErrorRequestByJSV2(
    std::shared_ptr<OHOS::NWeb::NWebJSAllSslErrorResult> result,
    ArkWebSslError error,
    const std::string& url,
    const std::string& originalUrl,
  ...
  bool flag = false;
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    flag = ark_web_handler_->OnAllSslErrorRequestByJSV2(
        nullptr, static_cast<int>(error), ArkWebStringClassToStruct(url),
        ArkWebStringClassToStruct(originalUrl),
        ArkWebStringClassToStruct(referrer), isFatalError, isMainFrame,
        stCertChainData);
...
void ArkWebHandlerWrapper::OnPageTitleV2(const std::string& title,
                                         bool isRealTitle) {
  ArkWebString stTitle = ArkWebStringClassToStruct(title);
...
void ArkWebHandlerWrapper::OnInsertBlanklessFrame(
    const std::string& pathToFrame) {
...
std::string ArkWebHandlerWrapper::OnHandleOverrideErrorPage(
  ...
  ArkWebString result = ark_web_handler_->OnHandleOverrideErrorPage(
      ark_web_request, ark_web_error);
...
void ArkWebHandlerWrapper::OnPdfLoadEvent(int32_t result,
                                          const std::string& url) {
  ArkWebString stUrl = ArkWebStringClassToStruct(url);
...
void ArkWebHandlerWrapper::OnTakeFocus(
    std::shared_ptr<OHOS::NWeb::NWebKeyEvent> event) {
  if (CHECK_SHARED_PTR_IS_NULL(event)) {
...
void ArkWebHandlerWrapper::OnInsertBlanklessFrameWithSize(
    const std::string& pathToFrame,
    uint32_t width,
    uint32_t height) {
  ArkWebString pathToFrame_ = ArkWebStringClassToStruct(pathToFrame);
...
void ArkWebHandlerWrapper::OnNativeEmbedObjectParamChange(
  ...
  ark_web_handler_->OnNativeEmbedObjectParamChange(
      new ArkWebNativeEmbedParamDataInfoImpl(paramDataInfo));
}
...
std::string ArkWebHandlerWrapper::OnWebNativeMessage(
    std::shared_ptr<OHOS::NWeb::NWebRuntimeConnectInfo> info,
  ...
  ArkWebString result = ark_web_handler_->OnWebNativeMessage(
      nweb_connect_info, new ArkWebNativeMessageCallbackImpl(callback));
  std::string connectInfo = ArkWebStringStructToClass(result);
  ArkWebStringStructRelease(result);
  return connectInfo;
}

void ArkWebHandlerWrapper::OnRefreshAccessedHistoryV2(const std::string& url,
                                                      bool is_reload,
                                                      bool isMainFrame) {
  ArkWebString stUrl = ArkWebStringClassToStruct(url);
