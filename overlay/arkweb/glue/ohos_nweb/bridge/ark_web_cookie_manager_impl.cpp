...
void ArkWebCookieManagerImpl::GetCookieAsync(
    ...
    ArkWebRefPtr<ArkWebStringValueCallback> callback) {
  if (CHECK_REF_PTR_IS_NULL(callback)) {
    nweb_cookie_manager_->GetCookieAsync(ArkWebStringStructToClass(url),
                                         incognitoMode, nullptr);
    return;
  }

  nweb_cookie_manager_->GetCookieAsync(
      ArkWebStringStructToClass(url), incognitoMode,
      std::make_shared<ArkWebStringValueCallbackWrapper>(callback));
}

int ArkWebCookieManagerImpl::SetCookieSync(const ArkWebString& url,
                                           const ArkWebString& value,
                                           bool incognitoMode,
                                           bool includeHttpOnly) {
  return nweb_cookie_manager_->SetCookieSync(ArkWebStringStructToClass(url),
                                             ArkWebStringStructToClass(value),
                                             incognitoMode, includeHttpOnly);
}

void ArkWebCookieManagerImpl::SetCookieAsync(
    const ArkWebString& url,
    const ArkWebString& value,
    bool incognitoMode,
    bool includeHttpOnly,
  ...
  if (CHECK_REF_PTR_IS_NULL(callback)) {
    nweb_cookie_manager_->SetCookieAsync(
        ArkWebStringStructToClass(url), ArkWebStringStructToClass(value),
        incognitoMode, includeHttpOnly, nullptr);
    return;
  }
