...
ArkWebRefPtr<ArkWebNWeb> ArkWebEngineImpl::CreateNWeb(
    ArkWebRefPtr<ArkWebNWebCreateInfo> create_info) {
...
void ArkWebEngineImpl::SetUserAgentForHosts(const ArkWebString& userAgent,
                                            const ArkWebStringVector& hosts) {
  nweb_engine_->SetUserAgentForHosts(ArkWebStringStructToClass(userAgent),
                                     ArkWebStringVectorStructToClass(hosts));
}
...
void ArkWebEngineImpl::SetProxyOverride(
    const ArkWebStringVector& proxyUrls,
  ...
  nweb_engine_->SetProxyOverride(
      ArkWebStringVectorStructToClass(proxyUrls),
      ArkWebStringVectorStructToClass(proxySchemeFilters),
      ArkWebStringVectorStructToClass(bypassRules), reverseBypass,
      nweb_proxy_callback);
}

void ArkWebEngineImpl::RemoveProxyOverride(
    ArkWebRefPtr<ArkWebProxyChangedCallback> callback) {
  std::shared_ptr<OHOS::NWeb::NWebProxyChangedCallback> nweb_proxy_callback =
      std::make_shared<ArkWebProxyChangedCallbackImpl>(callback);
...
void ArkWebEngineImpl::SetWebDebuggingAccessAndPort(bool isEnableDebug,
                                                    int32_t port) {
  nweb_engine_->SetWebDebuggingAccessAndPort(isEnableDebug, port);
}

uint32_t ArkWebEngineImpl::AddBlanklessLoadingUrls(
    const ArkWebStringVector& urls) {
  return nweb_engine_->AddBlanklessLoadingUrls(
      ArkWebStringVectorStructToClass(urls));
}

void ArkWebEngineImpl::RemoveBlanklessLoadingUrls(
    const ArkWebStringVector& urls) {
  nweb_engine_->RemoveBlanklessLoadingUrls(
      ArkWebStringVectorStructToClass(urls));
}

void ArkWebEngineImpl::ClearBlanklessLoadingCache(
    const ArkWebStringVector& urls) {
  nweb_engine_->ClearBlanklessLoadingCache(
      ArkWebStringVectorStructToClass(urls));
}

ArkWebString ArkWebEngineImpl::CheckBlankOptEnable(const ArkWebString& url,
                                                   int32_t nweb_id) {
  return ArkWebStringClassToStruct(nweb_engine_->CheckBlankOptEnable(
      ArkWebStringStructToClass(url), nweb_id));
}
...
void ArkWebEngineImpl::SetWebDestroyMode(int32_t mode) {
  nweb_engine_->SetWebDestroyMode(
      static_cast<OHOS::NWeb::WebDestroyMode>(mode));
}

int32_t ArkWebEngineImpl::SetSiteIsolationMode(int32_t mode) {
  return static_cast<int32_t>(nweb_engine_->SetSiteIsolationMode(
      static_cast<ArkWebSiteIsolationMode>(mode)));
}
