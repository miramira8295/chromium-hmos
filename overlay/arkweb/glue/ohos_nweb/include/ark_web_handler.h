...
class ArkWebHandler : public virtual ArkWebBaseRefCounted {
 public:
   ...
   */
  /*--ark web()--*/
  virtual void OnPageLoadError(int error_code,
  ...
  virtual void OnPermissionRequest(
      ArkWebRefPtr<ArkWebAccessRequest> request) = 0;
   ...
   */
  /*--ark web()--*/
  virtual bool OnPromptDialogByJS(
      const ArkWebString& url,
      const ArkWebString& message,
  ...
  /*--ark web()--*/
  virtual bool OnSslErrorRequestByJS(
  ...
  virtual void OnFullScreenEnter(
      ArkWebRefPtr<ArkWebFullScreenExitHandler> handler) = 0;

  /*--ark web()--*/
  virtual bool OnDragAndDropData(const void* data,
                                 size_t len,
   ...
   */
  /*--ark web()--*/
  virtual void OnDataResubmission(
      ArkWebRefPtr<ArkWebDataResubmissionCallback> handler) = 0;
  ...
  /*--ark web()--*/
  virtual bool OnDragAndDropDataUdmf(
      ArkWebRefPtr<ArkWebDragData> drag_data) = 0;
  ...
  /*--ark web()--*/
  virtual void OnGetTouchHandleHotZone(
      ArkWebRefPtr<ArkWebTouchHandleHotZone> hot_zone) = 0;
  ...
  /*--ark web()--*/
  virtual void OnOverScrollFlingVelocity(float x_velocity,
                                         float y_velocity,
                                         bool is_fling) = 0;
  ...
  /*--ark web()--*/
  virtual void OnShowAutofillPopup(const float offsetX,
                                   const float offsetY,
   ...
   */
  /*--ark web()--*/
  virtual void KeyboardReDispatch(ArkWebRefPtr<ArkWebKeyEvent> event,
                                  bool isUsed) = 0;
   ...
   */
  /*--ark web()--*/
  virtual void OnAdsBlocked(const ArkWebString& url,
                            const ArkWebStringVector& adsBlocked) = 0;
   ...
   */
  /*--ark web()--*/
  virtual void OnCursorUpdate(double x,
                              double y,
                              double width,
                              double height) = 0;
   ...
   */
  /*--ark web()--*/
  virtual void ReportDynamicFrameLossEvent(const ArkWebString& sceneId,
                                           bool isStart) = 0;
  ...
  /*--ark web()--*/
  virtual void OnNativeEmbedVisibilityChange(const ArkWebString& embed_id,
                                             bool visibility) = 0;
   ...
   */
  /*--ark web()--*/
  virtual bool OnSslErrorRequestByJSV2(
      ArkWebRefPtr<ArkWebJsSslErrorResult> result,
      int error,
   ...
   * @Input accessibilityId: the accessibility id of the accessibility node of
  ...
  /*--ark web()--*/
  virtual void OnShowAutofillPopupV2(const float offsetX,
                                     const float offsetY,
                                     const float height,
                                     const float width,
                                     const ArkWebStringVector& menu_items) = 0;
   ...
   */
  /*--ark web()--*/
  virtual void OnAccessibilityEventV2(int64_t accessibilityId,
                                      int32_t eventType,
                                      const ArkWebString& argument) = 0;

  /*--ark web()--*/
  virtual bool OnNestedScroll(float& x,
                              float& y,
                              float& xVelocity,
                              float& yVelocity,
                              bool& isAvailable) = 0;
   ...
   * @param url String: The url of the page requesting.
   * @param message String: The message of the dialog.
   * @param isReload bool: The isReload parameter is set to true when the page
   * is refreshed; otherwise, it remains false. Default is false.
   * @param result std::shared_ptr<NWebJSDialogResult>: A NWebJSDialogResult to
   *        confirm that the user closed the window.
   * @return To show a custom dialog, the app should return true.
   */
  ...
  /**
   * @Description: Called when an mouse native event occurs on native embed
   * area.
   * @Input mouse_event: Mouse events that contain information about the same
   * layer.
   */
  /*--ark web()--*/
  virtual void OnNativeEmbedMouseEvent(
      ArkWebRefPtr<ArkWebNativeEmbedMouseEvent> mouse_event) = 0;

  /**
   * @brief called when the web page is active for window.open called by other
   * web component.
   */
  /*--ark web()--*/
  ...
  /**
   * @Description Notify the SDK that a web site has started loading. This
   * method is called once for each main frame load. Embedded frame changes,
   * i.e. clicking a link whose target is an iframe and fragment navigations
  ...
  /**
   * @Description Notify the SDK that a web site has finished loading. This
   * method is called only for main frame. Different from onPageEnd,
   * onLoadFinished is triggered only once if the mainframe is automatically
   * redirected before the page is completely loaded. OnPageEnd is triggered
   * every navigation. fragment navigation also triggers onLoadFinished.
   *
  ...
  /**
   * @Description Called when an SSL error occurs during the loading of
   * resources (for the main frame and subframes).
   * @Input result: handler of result.
   * @Input error: error code.
   ...
   */
  /*--ark web()--*/
  virtual bool OnAllSslErrorRequestByJSV2(
      ArkWebRefPtr<ArkWebJsAllSslErrorResult> result,
      int error,
      const ArkWebString& url,
      const ArkWebString& originalUrl,
   ...
   *
   * @param title The document title.
   * @param isRealTitle Mark the source of the title. If it is true, the title
   * is derived from the H5 title element; If it is false, it is calculated from
   * the URL. By default, it is calculated from the URL.
   */
  /*--ark web()--*/
   ...
   *
   * @param pathToFrame The file used to insert frame. If empty, means remove
   * frame.
   */
  /*--ark web()--*/
   ...
   *
   * @param visible the soft keyboard panel status. If true, it represents the
   * keyboard display; If it is false, it indicates that the keyboard is hidden.
   *
   */
  /*--ark web()--*/
   ...
   *
   * @param data_info The information containing the set of all changed params
   * for the native object.
   *
   */
  /*--ark web()--*/
  virtual void OnNativeEmbedObjectParamChange(
      ArkWebRefPtr<ArkWebNativeEmbedParamDataInfo> data_info) = 0;
  ...
  /**
   * @brief Initiates webExtensions-native messaging requests and handles
   * corresponding callbacks
   *
   * @param info The runtime's message connect info.
   * @param callback Response handler for processing asynchronous replies from
   * native host.
  ...
  virtual ArkWebString OnWebNativeMessage(
      ...
      ArkWebRefPtr<ArkWebNativeMessageCallback> callback) = 0;
   ...
   */
  /*--ark web()--*/
  virtual void OnRefreshAccessedHistoryV2(const ArkWebString& url,
                                          bool isReload,
                                          bool isMainFrame) = 0;
};
