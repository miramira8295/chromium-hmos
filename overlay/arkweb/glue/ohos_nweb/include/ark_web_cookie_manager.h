...
class ArkWebCookieManager : public virtual ArkWebBaseRefCounted {
 public:
   ...
   */
  /*--ark web()--*/
  virtual int SetCookie(const ArkWebString& url,
   ...
   */
  /*--ark web()--*/
  virtual void SetCookie(const ArkWebString& url,
                         const ArkWebString& value,
                         ArkWebRefPtr<ArkWebBoolValueCallback> callback) = 0;
   ...
   */
  /*--ark web()--*/
  virtual ArkWebString ReturnCookie(const ArkWebString& url,
                                    bool& is_valid,
                                    bool incognito_mode) = 0;
   ...
   */
  /*--ark web()--*/
  virtual void ConfigCookie(const ArkWebString& url,
                            const ArkWebString& value,
   ...
   * @Input incognitoMode: true if web is in the incognito mode, false
   * otherwise.
   * @Input callback: a callback which is executed when the cookies have been
   * gotten.
   * @Since: 12005
   */
  /*--ark web()--*/
   ...
   * @Input value: the cookie as a string, using the format of the 'Set-Cookie'
   * HTTP response header.
   * @Input incognitoMode: true if web is in the incognito mode, false
   * otherwise.
   ...
   */
  /*--ark web()--*/
  virtual int SetCookieSync(const ArkWebString& url,
                            const ArkWebString& value,
                            bool incognitoMode,
                            bool includeHttpOnly) = 0;

  /**
   * @Description: Sets a single cookie (key-value pair) for the given URL
   * async.
   *
   * @Input url: the URL for which the cookie is to be set.
   * @Input value: the cookie as a string, using the format of the 'Set-Cookie'
