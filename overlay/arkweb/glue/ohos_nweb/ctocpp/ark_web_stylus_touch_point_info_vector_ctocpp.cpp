...
std::vector<std::shared_ptr<OHOS::NWeb::NWebStylusTouchPointInfo>>
    ARK_WEB_CALLBACK ArkWebStylusTouchPointInfoVectorStructToClass(
        const ArkWebStylusTouchPointInfoVector& struct_value) {
  std::vector<std::shared_ptr<OHOS::NWeb::NWebStylusTouchPointInfo>>
      class_value;
  if (struct_value.size > 0) {
    for (int count = 0; count < struct_value.size; count++) {
      std::shared_ptr<OHOS::NWeb::NWebStylusTouchPointInfo>
          nweb_stylus_point_info =
              std::make_shared<ArkWebStylusTouchPointInfoWrapper>(
                  ArkWebStylusTouchPointInfoCToCpp::Invert(
