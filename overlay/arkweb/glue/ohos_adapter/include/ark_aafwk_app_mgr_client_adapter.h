...
class ArkAafwkAppMgrClientAdapter : public virtual ArkWebBaseRefCounted {
 public:
  /*--ark web()--*/
  virtual int StartRenderProcess(const ArkWebString& renderParam,
                                 int32_t ipcFd,
                                 int32_t sharedFd,
                                 int32_t crashFd,
                                 pid_t& renderPid) = 0;
  ...
  /*--ark web()--*/
  virtual int StartChildProcess(const ArkWebString& renderParam,
                                int32_t ipcFd,
                                int32_t sharedFd,
                                int32_t crashFd,
                                pid_t& renderPid,
                                const ArkWebString& processType) = 0;

  /*--ark web()--*/
  virtual void SaveBrowserConnect(
      ArkWebRefPtr<ArkAafwkBrowserHostAdapter> adapter) = 0;
};
