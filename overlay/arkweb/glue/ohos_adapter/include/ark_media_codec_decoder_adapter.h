...
class ArkDecoderCallbackAdapter : public virtual ArkWebBaseRefCounted {
 public:
  ...
  /*--ark web()--*/
  virtual void OnStreamChanged(int32_t width,
                               int32_t height,
                               double frameRate) = 0;
...
class ArkMediaCodecDecoderAdapter : public virtual ArkWebBaseRefCounted {
 public:
  ...
  virtual int32_t ConfigureDecoder(
      const ArkWebRefPtr<ArkDecoderFormatAdapter> format) = 0;
  ...
  virtual int32_t GetOutputFormatDec(
      ArkWebRefPtr<ArkDecoderFormatAdapter> format) = 0;
  ...
  virtual int32_t SetCallbackDec(
      const ArkWebRefPtr<ArkDecoderCallbackAdapter> callback) = 0;

  /*--ark web()--*/
  virtual int32_t SetDecryptionConfig(void* session, bool isSecure) = 0;

  /*--ark web()--*/
  virtual int32_t SetAVCencInfo(
      uint32_t index,
      const ArkWebRefPtr<ArkAudioCencInfoAdapter> cencInfo) = 0;
};
