...
class ArkIBufferConsumerListenerAdapter : public virtual ArkWebBaseRefCounted {
 public:
  ...
  virtual void OnBufferAvailable(
      ArkWebRefPtr<ArkSurfaceBufferAdapter> buffer) = 0;
};
...
class ArkWindowAdapter : public virtual ArkWebBaseRefCounted {
 public:
  ...
  /*--ark web()--*/
  virtual int32_t NativeWindowSetBufferGeometry(void* window,
                                                int32_t width,
                                                int32_t height) = 0;
  ...
  /*--ark web()--*/
  virtual void NativeWindowSurfaceCleanCacheWithPara(void* window,
                                                     bool cleanAll) = 0;
...
class ArkNativeImageAdapter : public virtual ArkWebBaseRefCounted {
 public:
   ...
   * @Output windowBuffer: Indicates the pointer to an
   * <b>OHNativeWindowBuffer</b> point.
   ...
   */
  /*--ark web()--*/
  virtual int32_t AcquireNativeWindowBuffer(void** windowBuffer,
                                            int* acquireFenceFd) = 0;

  /**
   * @Description: Converts an <b>OHNativeWindowBuffer</b> instance to an
   * <b>OH_NativeBuffer</b>.
   * @Input windowBuffer: Indicates the pointer to a <b>OHNativeWindowBuffer</b>
   * instance.
   ...
   */
  /*--ark web()--*/
  virtual int32_t GetNativeBuffer(void* windowBuffer, void** nativeBuffer) = 0;

  /**
   * @Description: Release the <b>OHNativeWindowBuffer</b> to the buffer queue
   * for reuse.
   * @Input windowBuffer: Indicates the pointer to an
   * <b>OHNativeWindowBuffer</b> instance.
   * @Input fenceFd: Indicates a file descriptor handle, which is used for
   * timing synchronization.
   * @Return：Returns an error code, 0 is sucess, otherwise, failed.
   * @Since 12005
   */
  /*--ark web()--*/
  virtual int32_t ReleaseNativeWindowBuffer(void* windowBuffer,
                                            int fenceFd) = 0;
   ...
   */
  /*--ark web()--*/
  virtual void GetNativeWindowBufferSize(void* windowBuffer,
                                         uint32_t* width,
                                         uint32_t* height) = 0;
};
