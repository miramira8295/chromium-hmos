...
ArkEventHandlerFDListenerAdapterCppToC::
  ...
  std::call_once(flag, [] {
    ...
    funcMemberMap["nYmKyIhyoDzrNg1WeSk8Zw"] = reinterpret_cast<void*>(
        ark_event_handler_fdlistener_adapter_on_readable);
    ArkWebAdapterWebcoreBridgeHelper::GetInstance().RegisterFuncMember(
        ARK_EVENT_HANDLER_FDLISTENER_ADAPTER, funcMemberMap);
  });
}
