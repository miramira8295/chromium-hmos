...
ark_mmilistener_adapter_on_device_added(struct _ark_mmilistener_adapter_t* self,
                                        int32_t deviceId,
...
ArkMMIListenerAdapterCppToC::ArkMMIListenerAdapterCppToC() {
  ...
  std::call_once(flag, [] {
    ...
    funcMemberMap["dQqXrgZo$hudhbWGMlVJTA"] =
        reinterpret_cast<void*>(ark_mmilistener_adapter_on_device_added);
    funcMemberMap["u_0aQgEpeUYuS22Bg$lpNA"] =
        reinterpret_cast<void*>(ark_mmilistener_adapter_on_device_removed);
    ArkWebAdapterWebcoreBridgeHelper::GetInstance().RegisterFuncMember(
        ARK_MMILISTENER_ADAPTER, funcMemberMap);
  });
}
...
template <>
ArkWebBridgeType
    ArkWebCppToCRefCounted<ArkMMIListenerAdapterCppToC,
