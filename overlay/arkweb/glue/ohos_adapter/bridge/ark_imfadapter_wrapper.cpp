...
bool ArkIMFAdapterWrapper::Attach(
  ...
  if (!CHECK_SHARED_PTR_IS_NULL(listener) &&
    ...
    return ctocpp_->Attach(
        new ArkIMFTextListenerAdapterImpl(listener), isShowKeyboard,
...
bool ArkIMFAdapterWrapper::AttachWithRequestKeyboardReason(
    std::shared_ptr<OHOS::NWeb::IMFTextListenerAdapter> listener,
    bool isShowKeyboard,
    const std::shared_ptr<OHOS::NWeb::IMFTextConfigAdapter> config,
    bool isResetListener,
  ...
  if (!CHECK_SHARED_PTR_IS_NULL(listener) &&
    ...
    return ctocpp_->AttachWithRequestKeyboardReason(
        ...
        requestKeyboardReason);
             ...
             CHECK_SHARED_PTR_IS_NULL(config)) {
    return ctocpp_->AttachWithRequestKeyboardReason(nullptr, isShowKeyboard,
                                                    nullptr, isResetListener,
                                                    requestKeyboardReason);
  } else if (CHECK_SHARED_PTR_IS_NULL(listener)) {
    return ctocpp_->AttachWithRequestKeyboardReason(
        nullptr, isShowKeyboard, new ArkIMFTextConfigAdapterImpl(config),
        isResetListener, requestKeyboardReason);
  } else {
    return ctocpp_->AttachWithRequestKeyboardReason(
        new ArkIMFTextListenerAdapterImpl(listener), isShowKeyboard, nullptr,
        isResetListener, requestKeyboardReason);
  }
}
...
void ArkIMFAdapterWrapper::OnSelectionChange(std::u16string text,
                                             int start,
                                             int end) {
...
bool ArkIMFAdapterWrapper::SendPrivateCommand(const std::string& commandKey,
