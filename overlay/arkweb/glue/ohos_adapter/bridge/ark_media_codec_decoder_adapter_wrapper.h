...
class ArkMediaCodecDecoderAdapterWrapper
 ...
 public:
  explicit ArkMediaCodecDecoderAdapterWrapper(
      ArkWebRefPtr<ArkMediaCodecDecoderAdapter>);

  OHOS::NWeb::DecoderAdapterCode CreateVideoDecoderByMime(
      const std::string& mimetype) override;

  OHOS::NWeb::DecoderAdapterCode CreateVideoDecoderByName(
  ...
  OHOS::NWeb::DecoderAdapterCode QueueInputBufferDec(
      ...
      int32_t size,
      int32_t offset,
      OHOS::NWeb::BufferFlag flag) override;
  ...
  OHOS::NWeb::DecoderAdapterCode SetDecryptionConfig(void* session,
                                                     bool isSecure) override;

  OHOS::NWeb::DecoderAdapterCode SetAVCencInfo(
      uint32_t index,
      const std::shared_ptr<NWeb::AudioCencInfoAdapter> cencInfo) override;
