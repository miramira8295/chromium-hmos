...
class ArkMigrationManagerAdapterWrapper
    : public OHOS::NWeb::MigrationManagerAdapter {
 public:
  explicit ArkMigrationManagerAdapterWrapper(
      ArkWebRefPtr<ArkMigrationManagerAdapter>);

  void SetMigrationParam(const std::string& bundleName,
  ...
  uint32_t RegisterMigrationListener(
      std::shared_ptr<OHOS::NWeb::MigrationListenerAdapter> listener) override;
