...
class ArkScreenCaptureAdapterWrapper : public OHOS::NWeb::ScreenCaptureAdapter {
 public:
  explicit ArkScreenCaptureAdapterWrapper(
      ArkWebRefPtr<ArkScreenCaptureAdapter>);

  int32_t Init(
      const std::shared_ptr<NWeb::ScreenCaptureConfigAdapter> config) override;
  ...
  std::shared_ptr<OHOS::NWeb::SurfaceBufferAdapter> AcquireVideoBuffer()
      override;

  int32_t ReleaseVideoBuffer() override;

  int32_t AcquireAudioBuffer(
      std::shared_ptr<OHOS::NWeb::AudioBufferAdapter> audiobuffer,
      NWeb::AudioCaptureSourceTypeAdapter type) override;

  int32_t ReleaseAudioBuffer(NWeb::AudioCaptureSourceTypeAdapter type) override;
