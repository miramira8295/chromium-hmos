...
OHOS::NWeb::VSyncErrorCode ArkVSyncAdapterWrapper::RequestVsync(
    void* data,
    OHOS::NWeb::NWebVSyncCb cb) {
  if (!ctocpp_) {
...
void ArkVSyncAdapterWrapper::SetOnVsyncEndCallback(
...
void ArkVSyncAdapterWrapper::SetScene(const std::string& sceneName,
                                      uint32_t state) {
  if (!ctocpp_) {
    return;
  }
