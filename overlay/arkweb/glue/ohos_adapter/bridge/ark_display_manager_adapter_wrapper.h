...
class ArkDisplayManagerAdapterWrapper
 ...
 public:
  explicit ArkDisplayManagerAdapterWrapper(
  ...
  uint32_t RegisterFoldStatusListener(
      std::shared_ptr<OHOS::NWeb::FoldStatusListenerAdapter> listener) override;

  bool UnregisterFoldStatusListener(uint32_t id) override;

  std::shared_ptr<OHOS::NWeb::DisplayAdapter> GetPrimaryDisplay() override;

  std::vector<std::shared_ptr<OHOS::NWeb::DisplayAdapter>> GetAllDisplays()
      override;
