...
class ArkDrmCallbackAdapterImpl : public ArkDrmCallbackAdapter {
 public:
  ...
  void OnSessionMessage(const ArkWebString& sessionId,
                        int32_t& type,
                        const ArkWebUint8Vector& message) override;

  void OnProvisionRequest(const ArkWebString& defaultUrl,
                          const ArkWebString& requestData) override;

  void OnProvisioningComplete(bool success) override;

  void OnMediaKeySessionReady(void* session) override;

  void OnPromiseRejected(uint32_t promiseId,
                         const ArkWebString& errorMessage) override;

  void OnPromiseResolved(uint32_t promiseId) override;

  void OnPromiseResolvedWithSession(uint32_t promiseId,
                                    const ArkWebString& sessionId) override;

  void OnSessionClosed(const ArkWebString& sessionId) override;

  void OnSessionKeysChange(const ArkWebString& sessionId,
                           const ArkWebStringVector& keyIdArray,
                           const ArkWebUint32Vector& statusArray,
                           bool hasAdditionalUsableKey,
                           bool isKeyRelease) override;

  void OnSessionExpirationUpdate(const ArkWebString& sessionId,
                                 uint64_t expirationTime) override;

  void OnStorageProvisioned() override;

  void OnStorageSaveInfo(const ArkWebUint8Vector& ketSetId,
