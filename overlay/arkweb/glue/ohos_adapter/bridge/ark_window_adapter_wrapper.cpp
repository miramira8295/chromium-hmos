...
ArkWebNativeWindow ArkWindowAdapterWrapper::CreateNativeWindowFromSurface(
    void* pSurface) {
  if (!ctocpp_) {
    return nullptr;
  }
...
void ArkWindowAdapterWrapper::NativeWindowSurfaceCleanCacheWithPara(
    ArkWebNativeWindow window,
    bool cleanAll) {
  if (!ctocpp_) {
    return;
...
void ArkWindowAdapterWrapper::SetTransformHint(uint32_t rotation,
                                               ArkWebNativeWindow window) {
  if (!ctocpp_) {
    return;
  }
