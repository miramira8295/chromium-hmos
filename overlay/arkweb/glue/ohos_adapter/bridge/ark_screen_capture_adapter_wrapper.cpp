...
int32_t ArkScreenCaptureAdapterWrapper::AcquireAudioBuffer(
    ...
    NWeb::AudioCaptureSourceTypeAdapter type) {
  if (CHECK_SHARED_PTR_IS_NULL(audiobuffer)) {
    return ctocpp_->AcquireAudioBuffer(nullptr, (int32_t)type);
  }

  return ctocpp_->AcquireAudioBuffer(new ArkAudioBufferAdapterImpl(audiobuffer),
                                     (int32_t)type);
}

int32_t ArkScreenCaptureAdapterWrapper::ReleaseAudioBuffer(
    NWeb::AudioCaptureSourceTypeAdapter type) {
